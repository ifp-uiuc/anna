...
def compute_overall_recon_error(model, mode=None):
    ...
    for x_batch, y_batch in iterator:                
        ...
        recon_error[i] = model.eval(x_batch/2)
        print('Mini-batch {}: {}'.format(i+1, recon_error[i]))
        i+=1
...
if __name__ == "__main__":
    ...
    #compute_overall_recon_error(model, 'train')
    compute_overall_recon_error(model, 'test')