...
class SupervisedDataset(object):
    ...
    def iterator(self, mode='sequential', batch_size=None, num_batches=None, rng_seed=0):
        ...
        if(mode == 'sequential'):
            ...
            self.iter = DatasetIteratorSequential(self.X, self.y, batch_size, num_batches, self.n_samples)   

            
        elif(mode == 'random_uniform'):
        
            if batch_size is None:
                raise ValueError("batch_size cannot be None for random uniform iteration")
            elif num_batches is None:
                raise ValueError("num_batches cannot be None for random uniform iteration")
            self.iter = DatasetIteratorRandomUniform(self.X, self.y, batch_size, num_batches, self.n_samples, rng_seed)
            ...
            self.iter = DatasetIteratorRandomUniformNoRep(self.X, self.y, batch_size, num_batches, self.n_samples, rng_seed)
        
        else:
            raise ValueError("please specify the iterator mode as either ('sequential', 'random_uniform', 'random_uniform_no_rep')")
...
class BasicIterator(object):    
    ...
    def __init__(self, X, y, batch_size=None, num_batches=None, num_samples=None):
...
class DatasetIteratorSequential(BasicIterator):
    ...
    def __init__(self, X, y, batch_size=None, num_batches=None, num_samples=None):
        ...
        super(DatasetIteratorSequential, self).__init__(X, y, batch_size, num_batches, num_samples)
...
class DatasetIteratorRandomUniform(BasicIterator):
    ...
    def __init__(self, X, y, batch_size=None, num_batches=None, num_samples=None, rng_seed=0):
        #print('Using Random Uniform Iterator (with replacement)')
        np.random.seed(rng_seed)
        super(DatasetIteratorRandomUniform, self).__init__(X, y, batch_size, num_batches, num_samples)
    ...
    def next(self):
        if self.batch_count >= self.num_batches:
            ...
            self.last = np.random.random_integers(low=0,high=self.num_samples - 1,size=(self.batch_size,))          
            self.batch_count += 1
            return self.X[self.last, :, :, :], self.y[self.last]
...
class DatasetIteratorRandomUniformNoRep(BasicIterator):
    ...
    def __init__(self, X, y, batch_size=None, num_batches=None, num_samples=None, rng_seed=0):
        ...
        super(DatasetIteratorRandomUniformNoRep, self).__init__(X, y, batch_size, num_batches, num_samples)
...
if __name__ == "__main__":
    ...
    #d.iterator(mode='random_uniform', batch_size=128, num_batches = 1000, rng_seed=10)
    d.iterator(mode='random_uniform_no_rep', batch_size=128)

    for i, [b, b_l] in enumerate(d):
        print('Batch {} --- {}'.format(i+1, b.shape))
        print('Batch Labels {}'.format(b_l.shape))        
