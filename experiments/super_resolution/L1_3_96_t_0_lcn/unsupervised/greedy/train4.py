...
model = RegressionModel2('experiment', './', learning_rate=0.00002)
...
shape = x_batch.shape
c, x, y, b = shape
y_batch = numpy.zeros(shape, dtype=numpy.float32)
x_batch_zeros = numpy.zeros((16, x, y, b), dtype=numpy.float32)
count = 0
for x_batch in train_iterator:
    ...
    for i in range(128):
        ...
        y_batch[:, :, :, i] = numpy.float32(im_degrade).transpose(2,0,1)
    x_batch_zeros[0:3, :, :, :] = x_batch
    error = model.train(y_batch, x_batch_zeros) # x and y are flipped
    monitor.stop(error) 
    # recon_visualizer.run()   
    filter_visualizer.run()
    # if count % 100 == 0:
    #     gah = model.prediction(y_batch)
    #     im = x_batch[:, :, :, 0].transpose(1,2,0)
    #     im_degrade = y_batch[:, :, :, 0].transpose(1,2,0)
    #     im_nn = gah[0:3, :, :, 0].transpose(1,2,0)
    #     show_im(numpy.uint8(im), 
    #             numpy.uint8(im_degrade),
    #             numpy.uint8(im_nn))
    # count += 1