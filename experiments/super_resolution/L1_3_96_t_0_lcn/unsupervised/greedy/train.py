...
model = RegressionModel('experiment', './', learning_rate=0.001)
...
for x_batch in train_iterator:
    ...
    error = model.train(y_batch, x_batch) # x and y are flipped
    monitor.stop(error) 
    # recon_visualizer.run()   
    filter_visualizer.run()
    # if count % 100 == 0:
    #    gah = numpy.array(model.prediction(y_batch))
    #    im = x_batch[:, :, :, 0].transpose(1,2,0)
    #    im_degrade = y_batch[:, :, :, 0].transpose(1,2,0)
    #    im_nn = gah[:, :, :, 0].transpose(1,2,0)
